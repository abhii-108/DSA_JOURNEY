...
# 3 / 2 = 2 hours. Though mathematically, he should take 1.5 hrs but it is clearly stated in the question that after completing a pile Koko will not consume more bananas in that hour. 
...
def helper(arr,b_hr, h):

    curr_time = 0 

    for x in arr:
        eat_time =  math.ceil(x/b_hr)
        curr_time += eat_time
...
def koko_eating_banana(arr,N,h):

    # first we need to find min and max of array 
    
    left = min(arr)
    right = max(arr)
    print(f' min arr {left} , max arr {right}')
    res = -1 
    while left <= right:
        mid = left + (right - left) // 2

        mid_op = helper(arr,mid,h)
...
print(koko_eating_banana([25, 12, 8, 14, 19],5,5))
print(koko_eating_banana([7, 15, 6, 3],4,8))
